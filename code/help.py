...
def on_title(win):
    ...
    if (
        is_context_help_showing
        and live_update
        and win.title != cached_window_title
        and selected_context == None
    ):
...
def gui_context_help(gui: imgui.GUI):
    ...
    if selected_context is None:
        total_page_count = int(
            math.ceil(len(sorted_context_map_keys) / max_items_per_page)
        )

        if not show_enabled_contexts_only:
            gui.text(
                "Context help ({}/{})".format(current_context_page, total_page_count)
            )
        else:
            gui.text(
                "Active Context help ({}/{})".format(
    ...
    # if there's a selected context, draw the commands for it
        ...
        total_page_count = int(
            math.ceil(len(context_mapping[selected_context]) / max_items_per_page)
        )
        gui.text(
            "{} ({}/{})".format(
